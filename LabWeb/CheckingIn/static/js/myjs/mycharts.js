...
function AddPoint(chartname, datas, firstin){          
    ...
    else
    {
    	...
    	x = new Date(Date.parse(datas[0]['receivetime'])).getTime();
    	y = datas[0][chartname];
    	latex = 0;
...
function AddPoints(onlyname){
	$.ajax(
	{
		type: "get",
		url: "/display_charts_ajax",
		data: {onlyname:onlyname, type:'one'},
		dataType: "json",
		contentType: "application/json; charset=utf-8",
		success: function(data)
		{
			$('.chart').each(function(){
				AddPoint($(this).attr('id'), data, true);
			});
			WhatIsSomeoneDoing();
		},
//		error: function()
//		{
//			alert("链接错误，请联系服务器管理员");
//		}
	});
	
}
function ChartDatas(onlyname)
{
	$.ajax(
	{
		type: "get",
		url: "/display_charts_ajax",
		data: {onlyname:onlyname, type:'all'},
		dataType: "json",
		contentType: "application/json; charset=utf-8",
		success: function(data)
		{
			$('.chart').each(function(){
				MakeCharts($(this).attr('id'), data);
			});
			WhatIsSomeoneDoing();
		},
//		error: function()
